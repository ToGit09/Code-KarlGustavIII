...
public:
     ...
     */
    movement_event ballanfahrt(us_sensor_event us, ir_sensor_event IR_Data, bool Cornerspeed, bool leftcorner, movement_event move = movement_event())
    {
        ...
        if (abs(IR_Data.Orbit_direction) < 150)
            move.AngleOfAttack = IR_Data.Orbit_direction;
        else if (us.dist_f < 28)
            move.AngleOfAttack = IR_Data.Orbit_direction * 1.4;
        else
    ...
    movement_event toranfahrt(bool PixyDrive, us_sensor_event US, compass_sensor_event Compass, camera_event cam, movement_event move = movement_event())
    {
        ...
        {
            if (US.dist_f > 25 || US.dist_b < 50) // Platz nach vorne zum schiessen
            {
            ...
            else // kein Platz nach vorne zum Tor fahren
            {
                ...
                if (US.offsetx < 8 && US.offsetx > -8) // wenn vorm Tor
                {
                    ...
                    move.angle = 0, move.speed = stdSpeed, move.AngleOfAttack = 160;
                    action.kick();
                }
                else
                {
...
public:
    ...
    void initialize(void)
    {
        ...
        // analogWriteFrequency(33, 600);
        //----------------------------------------------------------------------------------------//

        /*// analogWrite(33, 0); // Dribbler aus
        Tactics.action.setRGB(1, 255, 255, 0);
        Tactics.action.renderRGBs();
        delay(100);
        // delay(1000);
        //  analogWrite(33, 255);  // Motor 1 aus

        esc.attach(33);
        // motor.rotate(DRIBBLER, 0);
        // delay(2000);
        // motor.rotate(DRIBBLER, 100);
        esc.write(0);
        delay(5000);
        esc.write(20);
        delay(2000);
    ...
    void update(void)
    {
        ...
        if (ReadTimer3 >= 50) // BIN AUF 50 RUNTER war 100
        {
            LDR = Read.LDR();
            ReadTimer3 = 0;
        }

        // als test mal immer auf True/false und Zeit anschauen
    ...
    void system(void)
    {
        if (LoopTimer >= LoopTiming)
        {
            ...
            if (switches.MainSwitch)
            {
                ...
                // esc.write(50); // Dribbler an

                // Buttons im Spiel nicht abfragen um interferenzen zu vermeiden
            ...
            else
            {
                ...
                if (switches.SWI3)
                {
                    ...
                    Serial.print(",  Angle : ");
                    Serial.print(IR.Orbit_direction);
                    Serial.print(", Anfahrt : ");
                    Serial.println(IR.ballanfahrt_neuW);
                    // Serial.print(",  TSSP : ");
                    // Serial.println(IR.heading);
                }
                ...
                /*Serial.print("IR-TSOP: ");
                Serial.print(IR.heading);
                Serial.print(" : ");
                for (int i = 0; i < 8; i++)
                {
                    Serial.print(IR.TSOP[i]);
                    Serial.print(" ");
                }
                Serial.println();*/
    ...
    void gameLogik(void)
    {
        ...
        {                        // corner detection
            ...
            {
                cornerdetected = true;
                if (static_cast<unsigned long>(CornerTimer) > static_cast<unsigned long>(CornerTime) || cornerisUnset) // ne neue Ecke wurde erkannt
                // CornerTimer = 0;
                {
                    if (US.reliable)
                    {
                    ...
                    {
                        if (US.dist_l < US.dist_r)
                            isItLeftCorner = true, Serial.print("LEFT CORNER DETECTED UNRELIABLE");
                        else
                            isItLeftCorner = false, Serial.print("RIGHT CORNER DETECTED UNRELIABLE");
                        cornerisUnset = true;
                        ausrichten = true;
                    }
                ...
                if (static_cast<unsigned long>(CornerTimer) > static_cast<unsigned long>(CornerTime + Timeout))
                    CornerTimer = 0, cornerisUnset = true, ausrichten = true;

                if (cornerdetected || (static_cast<unsigned long>(CornerTimer) < static_cast<unsigned long>(CornerTime)))
                    amIinanCorner = true;
                ...
                if ((IR.Orbit_direction > 70 || IR.Orbit_direction < -70) || (US.dist_f > 40)) // Wenn der Ball neben oder hinter dem Robooter ist können wir nicht in der Ecke stehen -> Ballanfahrtspeed nicht verkleinern
                    amIinanCorner = false, cornerisUnset = true, CornerTimer = CornerTime + 1000UL;
            }
        ...
        if (IshoouldDefend)
            ...
            move = Tactics.ballanfahrt(US, IR, amIinanCorner, isItLeftCorner), StatusR = 255, StatusG = 0, StatusB = 0;
