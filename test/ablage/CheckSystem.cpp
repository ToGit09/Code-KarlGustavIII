...
void setup()
{
    Karl_GustavIII.initialize(); // Roboter initialisieren
}
...
void loop()
{
    ...
    // Alle an machen und Hell -> Dunkel
    for (int n = 100; n > 10; n -= 10)
    {
        RGBs.clear();
        RGBs.setBrightness(n);
        for (int i = 0; i < 10; i++)
        {
            RGBs.setPixelColor(i, 255, 255, 255);
        }
        RGBs.show();
        delay(200);
    }
    ...
    for (idx = 0; idx < 8; idx++) // Alle US Sensoren anfragen
    {
        ...
        if (rev != 0xFF) // anything that is not FF is a present sensor
        {
            ...
            RGBs.clear();
            RGBs.setPixelColor(0, 255 - USnum * 63, 0 + USnum * 63, 0);
            RGBs.show();
        }
        else
        {
    ...
    //-Motoren--------------------------------------------------------------------------------//
    RGBs.setPixelColor(2, 255, 0, 0);
    RGBs.show();

    Codeaction Action;

    Action.setMotor(0, 1, 100);
    delay(200);
    Action.setMotor(0, 2, 100);
    delay(200);
    Action.brakeMotor(0);
    delay(200); 

    RGBs.setPixelColor(2, 170, 85, 0);
    RGBs.show();

    Action.setMotor(1, 1, 100);
    delay(200);
    Action.setMotor(1, 2, 100);
    delay(200);
    Action.brakeMotor(1);
    delay(200); 

    RGBs.setPixelColor(2, 85, 170, 0);
    RGBs.show();

    Action.setMotor(2, 1, 100);
    delay(200);
    Action.setMotor(2, 2, 100);
    delay(200);
    Action.brakeMotor(2); 
    delay(200);

    RGBs.setPixelColor(2, 0, 255, 0);
    RGBs.show();

    Action.dribbler(true, true); // Dribbler an
    delay(200);
    Action.dribbler(false, true); // Dribbler aus   
    delay(200);
    Action.dribbler(true, false); // Dribbler an
    delay(200);
    Action.dribbler(false, false); // Dribbler aus
    delay(200);

    RGBs.setPixelColor(3, 0, 255, 0);
    RGBs.show();

    RGBs.setPixelColor(4, 255, 0, 0);
    RGBs.show();

    Action.kicker_reset(); // Kicker reset
    delay(500);
    Action.kick(); // Kicker schießen
    RGBs.setPixelColor(4, 170, 85, 0);
    RGBs.show();
    delay(500);
